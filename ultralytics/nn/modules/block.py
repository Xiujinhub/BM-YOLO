...
#     inner_channels = c2 // 4
#     return nn.Sequential(
#         nn.Conv2d(c1, inner_channels, kernel_size=patch_size, stride=patch_size),
#         nn.SiLU(),
...
#                                   padding=1, groups=inner_channels),
#                         nn.SiLU(),
#                         nn.GroupNorm(1, inner_channels)
#                     )
#                 ),
#                 nn.Conv2d(inner_channels, inner_channels, 1, stride=1, padding=0, groups=1),
#                 nn.SiLU(),
#                 nn.GroupNorm(1, inner_channels)
#             ) for _ in range(depth)
#         ],
#         nn.Conv2d(inner_channels, c2, 1, stride=1, padding=0),
#         nn.SiLU(),
#         nn.GroupNorm(1, c2)
#     )
#
# class MultiSEAM(nn.Module):
#     def __init__(self, c1, depth=1, kernel_size=3, patch_size=[3, 5],  # 减少分支数量
#                  reduction=16):
...
class DWSA(nn.Module):
    def __init__(self, c1, reduction=16):
        ...
        self.c1 = c1
        # 深度可分离卷积
        self.depthwise = nn.Conv2d(c1, c1, 3, padding=1, groups=c1)
        self.pointwise = nn.Conv2d(c1, c1, 1)
        self.act = nn.SiLU()
        self.norm = nn.GroupNorm(1, c1)
        # 极简通道注意力
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        self.fc = nn.Sequential(
            nn.Linear(c1, max(4, c1 // reduction), bias=False),
            nn.ReLU(inplace=True),
            nn.Linear(max(4, c1 // reduction), c1, bias=False),
            nn.Sigmoid()
        )
    def forward(self, x):
        identity = x
...
# 超轻量多尺度深度可分离注意力模块
class UltraLightMCEA(nn.Module):
    def __init__(self, c1, reduction=8, scales=[3, 5]):
        """
        超轻量多尺度深度可分离注意力模块
        :param c1: 输入输出通道数
        :param reduction: 通道压缩比例 (默认8)
        :param scales: 多尺度池化大小列表 (默认[3,5])
        """
        super().__init__()
        self.c1 = c1
        self.scales = scales
        # 深度可分离卷积核心
        self.dw_conv = nn.Conv2d(c1, c1, 3, padding=1, groups=c1, bias=False)
        self.pw_conv = nn.Conv2d(c1, c1, 1, bias=False)
        # 轻量级归一化
        ...
        for s in scales:
            pool = nn.Sequential(
                nn.AdaptiveAvgPool2d(s) if s > 1 else nn.Identity(),
                nn.Conv2d(c1, c1, 1, bias=False)  # 轻量级通道调整
            )
            self.pools.append(pool)
        # 极简通道注意力
        self.channel_attention = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(c1, max(4, c1 // reduction), 1, bias=False),
            nn.ReLU(inplace=True),
            nn.Conv2d(max(4, c1 // reduction), c1, 1, bias=False),
            nn.Sigmoid()
        )
        # 特征融合卷积
        self.fusion = nn.Conv2d(len(scales) * c1, c1, 1, bias=False)
        # 初始化权重
        self._initialize_weights()
    def _initialize_weights(self):
        # 深度卷积初始化为单位卷积
        nn.init.dirac_(self.dw_conv.weight)
        # 点卷积初始化为零（残差起点）
        nn.init.zeros_(self.pw_conv.weight)
        # 融合层初始化为均值融合
        with torch.no_grad():
            self.fusion.weight.fill_(1.0 / len(self.scales))
    def forward(self, x):
        identity = x
        # 深度可分离卷积
        x = F.silu(self.dw_conv(x))
        x = self.norm(x)
        x = self.pw_conv(x)
        # 多尺度特征提取（无计算开销）
        scale_features = []
        for pool in self.pools:
            ...
            pooled = pool(x)
            # 上采样回原尺寸
            if not isinstance(pool[0], nn.Identity):
                pooled = F.interpolate(pooled, x.shape[2:], mode='nearest')
            scale_features.append(pooled)
        # 融合多尺度特征
        fused = self.fusion(torch.cat(scale_features, dim=1))
        # 通道注意力
        attention = self.channel_attention(x + fused)
        # 残差连接
        return identity * attention + x


class lightMCEA(nn.Module):
...
# 通道压缩注意力机制
class CCEA(nn.Module):
    def __init__(self, c1, reduction=32, d=2):
        """
        修改后的多尺度增强通道注意力模块
        特征提取部分改为图片所示的多分支结构
        :param c1: 输入输出通道数
        ...
        self.dw_conv_branch = nn.Sequential(
            nn.Conv2d(self.expanded_channels, self.expanded_channels, kernel_size=3,
                      padding=1, groups=self.expanded_channels, bias=False),
            nn.BatchNorm2d(self.expanded_channels),
            nn.SiLU()
        )
        self.fuse_pw_conv = nn.Sequential(
            nn.Conv2d(self.expanded_channels, c1, kernel_size=1, bias=False),
            nn.SiLU(),
            nn.GroupNorm(num_groups=8, num_channels=c1)
        )
        self.gap = nn.AdaptiveAvgPool2d(1)
        self.compress = nn.Conv2d(c1, c1, kernel_size=1, bias=False)
        self.channel_attention = nn.Sequential(
            nn.Conv2d(c1, mid_channels, kernel_size=1, bias=False),
            nn.ReLU(inplace=True),
            nn.Conv2d(mid_channels, c1, kernel_size=1, bias=False),
    ...
    def forward(self, x):
        ...
        branch_dw = self.dw_conv_branch(out)
        branch_identity = out
        fused = branch_dw + branch_identity
        feature_extracted = self.fuse_pw_conv(fused)
        compressed_vector = self.compress(self.gap(feature_extracted))
        attention_weights = self.channel_attention(compressed_vector)
        return identity * attention_weights
