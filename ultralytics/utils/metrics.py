...
def bbox_iou(box1, box2, xywh=True, GIoU=False, DIoU=False, CIoU=False, SIoU=False, EIoU=False, WIoU=False,TIoU=False, Focal=False,
             delta=0.5, lambda_base=0.5, gamma=0.1, alpha=1, scale=False, eps=1e-7):
    # Returns Intersection over Union (IoU) of box1(1,4) to box2(n,4)

    """
    扩展的IoU计算函数，支持多种IoU变体，包括新添加的TIoU
    ...
    Args:
        ...
        TIoU (bool): 是否使用TIoU
        delta (float): TIoU参数δ
        lambda_base (float): TIoU参数λ基础值
        gamma (float): TIoU热分布模拟参数
        ... (其他参数保持不变)

    Returns:
        (torch.Tensor): IoU值 [n] 或 (根据TIoU设置返回元组)
    """
    ...
    if TIoU:
        ...
        # 确保在[0,1]范围内
        if scale:
            return tiou.clamp(min=0, max=1), thermal_dist  # 返回TIoU和thermal_dist用于损失缩放
...
def plot_pr_curve(px, py, ap, save_dir=Path("pr_curve.png"), names=(), on_plot=None):
    ...
    # 保存all这条曲线的xy # PR-Curve
    with open('runs/yolov10n.csv', 'w+') as f:
